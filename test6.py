...
def generate_user_data(index):
    return {
        ...
        'username': f'user950{index}',
        'email': f'user590{index}@example.com',
...
def register_user(index):
    ...
    try:
        ...
        print(f"[✘] Error with user {data['username']}: {e}")
    finally:
        driver.quit()

def main():
    total_users = 500
    max_threads = 5  # You can increase based on CPU and internet
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        executor.map(register_user, range(1, total_users + 1))
